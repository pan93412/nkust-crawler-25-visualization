...
from models import ArticleMongoModel, CommentMongoModel, ReplyMongoModel, comment_from_mongo_model, reply_from_mongo_model
import nlp
...
comments = comments_collection.find({"article_id": article["_id"]}).skip(skip).limit(items_per_page).sort("created_at", -1)
...
comments_display_df_state = st.dataframe(comments_display_df, selection_mode="single-row", on_select="rerun")
...
if comment_selection and "rows" in comment_selection and len(comment_selection["rows"]) > 0:
    st.divider()
    ...
    replies = replies_collection.find({"article_id": article["_id"], "comment_id": ObjectId(comment["_id"])}).to_list()
    ...
    replies_display_df = replies_df[["reaction_type", "content", "author", "likes", "dislikes", "created_at"]]
